...
is( $calc->_overdue_days($order), 0, '반납예정일(D-Day)' );
...
is( $calc->_overdue_days($order), 1,    '반납예정일(-1d)' );
...
is( $calc->_overdue_days($order), 2,     '반납예정일(-2d)' );
